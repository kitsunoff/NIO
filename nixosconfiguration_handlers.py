...
async def apply_nixos_configuration(
    ...
    try:
        ...
        if needs_full_install and not is_remove:
            ...
            env_for_cmd = (
                os.environ.copy()
            )  # nixos-anywhere может не читать NIX_SSHOPTS
        else:
...
async def reconcile_nixos_configuration(body, spec, name, namespace, **kwargs):
    ...
    try:
        ...
        repo_name, repo_url, commit_type = parse_flake_reference(
            spec["gitRepo"] + spec["flake"]
        )
        if commit_type == "floating":
            ...
            new_commit_hash = await get_remote_commit_hash(
                repo_url, ref, spec.get("credentialsRef"), namespace
            )
        else:
        ...
        try:
            ...
            success = await apply_nixos_configuration(
                ...
                bool(
                    deletion_timestamp
                ),  # is_remove = True если есть deletionTimestamp
                needs_full_install,
            )
                ...
                if deletion_timestamp:
                    ...
                    await update_configuration_status(
                        ...
                        namespace,
                        {
                            ...
                            or current_has_full_install,  # устанавливаем или сохраняем флаг
                            "conditions": [
                                {
                                    "type": "Applied",
                                    "status": "True",
...
async def check_floating_references(body, spec, name, namespace, **kwargs):
    ...
    try:
        current_head = await get_remote_commit_hash(
            repo_url, ref, spec.get("credentialsRef"), namespace
        )
        ...
        if current_commit and current_commit != current_head:
            logger.info(
                f"Floating reference {flake_ref} changed from {current_commit} to {current_head}, triggering reconcile"
            )
