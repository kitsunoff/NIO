...
async def prepare_configuration_tarball(config_spec: Dict, namespace: str) -> str:
    ...
    try:
        ...
        repo_path, commit_hash = await clone_git_repo(
            config_spec["gitRepo"], config_spec.get("credentialsRef"), namespace
        )
...
async def prepare_additional_files(
    config_spec: Dict, namespace: str, repo_path: str
) -> None:
        ...
        for file_spec in config_spec["additionalFiles"]:
            ...
            if file_spec["valueType"] == "Inline":
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to process secret {file_spec['secretRef']['name']}: {e}"
                    )
                    raise
                ...
                logger.info(
                    f"NixosFacter file will be handled by flake: {file_spec['path']}"
                )


def copy_file_to_pod(
    namespace: str, pod_name: str, source_file: pathlib.Path, dest_path: str
):
...
async def create_nixos_job(
    ...
    try:
                        ...
                        "volumes": [
                            {
                                "name": "ssh-key-volume",
                                "secret": {
                                    "secretName": machine_spec["sshKeySecretRef"][
                                        "name"
                                    ],
                                    "defaultMode": 0o600,
                                },
        ...
        logger.info(
            f"Created Job {job_name} for configuration application with SSH key"
        )
...
async def wait_for_job_completion(
    job_name: str, namespace: str, timeout: int = 1800
) -> bool:
...
async def handle_configuration_create_with_job(body, spec, name, namespace, **kwargs):
    ...
    try:
        ...
        is_discoverable = await check_machine_discoverable(
            machine["spec"], body, machine_name, namespace
        )
        if not is_discoverable:
            logger.warning(
                f"Skipping configuration application for {name}: machine {machine_name} is not discoverable"
            )
        ...
        job_created = await create_nixos_job(job_name, namespace, machine["spec"], spec)
        ...
        if success:
            ...
            logger.info(
                f"Successfully applied configuration {name} to machine {machine_name} via Job"
            )
        else:
...
async def handle_configuration_delete_with_job(body, spec, name, namespace, **kwargs):
    ...
    try:
        ...
        if "onRemoveFlake" in spec:
            ...
            tarball_path, commit_hash = await prepare_configuration_tarball(
                spec, namespace
            )
            temp_dir = os.path.dirname(tarball_path)
            ...
            job_name = (
                f"nixos-remove-{name}-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
            )
            job_created = await create_nixos_job(
                job_name, namespace, machine["spec"], spec, is_remove=True
            )
